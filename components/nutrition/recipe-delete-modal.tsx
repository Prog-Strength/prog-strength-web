 ...
 * PantryItemDeleteModal: log entries that used this recipe keep their
 * numbers because the API freezes macros at log time. Stays open on
 * server error.
 */
export function RecipeDeleteModal({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="recipe-delete-modal-title"
    >
      <div
      ...
      <div className="relative flex w-full max-w-sm flex-col overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--background)] shadow-xl">
        <header className="flex items-center justify-between gap-3 border-b border-[var(--border)] px-5 py-3">
          <h2 id="recipe-delete-modal-title" className="text-base font-semibold">
            Delete recipe?
          </h2>
          <button
            type="button"
        ...
        <div className="flex flex-col gap-3 px-5 py-4">
          <p className="text-sm">
            Delete the recipe <span className="font-medium">{recipe.name}</span>? Log entries that
            used this recipe keep their numbers — macros were frozen at log time.
          </p>
